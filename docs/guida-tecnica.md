...
Gli elenchi sono due: **`PedalRegistry.h`** per le sei sezioni della scheda MAIN e **`FxRegistry.h`** per le cinque della scheda FX, con `FxDSP.h` accanto. Stessa forma, stessi tipi di comando, contenuto diverso — e riserve di lunghezza diversa, otto comandi per il registro principale e cinque per quello FX, che non ha modelli cosi' ricchi. `ModelView` nell'editor appiattisce i due registri in una vista sola, cosi' il resto del codice non deve sapere da quale elenco venga il pedale che sta mostrando.

**Un effetto in una sezione sola.** Scegliendo a mano un modello gia' presente in un'altra sezione della stessa famiglia, la scelta viene rifiutata: `slotWithModel()` lo trova, il parametro torna al valore di prima e compare un avviso che dice dove sta gia'. Il controllo scatta solo per le scelte fatte a mano: uno stato salvato o un preset si caricano com'e', anche se contenessero un doppione. Il modello neutro "Bypass / Clean" e' esente, perche' non e' un effetto.
...
Come si e' lasciato un pedale -- valori dei pomelli, interruttore e accensione -- e' invece del pedale, non della sezione. `modelMemory_` lo tiene per **modello** e non per slot, perche' deve funzionare anche quando il pedale viene prima tolto di dove stava e poi ripreso altrove: al momento in cui lo si richiama, la sezione di partenza non lo ha piu'. I valori memorizzati sono quelli normalizzati 0..1 della riserva, quindi si spostano fra sezioni senza conversioni.

`applyRememberedState()` scrive nello slot solo se quel modello ha una memoria; altrimenti la sezione tiene la sua accensione e i pomelli partono dai valori di fabbrica del pedale.

**Quale copia comanda.** Con lo stesso pedale in due sezioni, la memoria deve seguire quella che l'utente sta regolando, non l'ultima che capita nel ciclo: `slotSeen_` tiene l'istantanea di ogni sezione al giro precedente e solo la sezione che e' effettivamente cambiata aggiorna la memoria del modello. Senza questo, una copia ferma con i valori vecchi sovrascriveva quella appena regolata, e il pedale si ritrovava altrove com'era prima delle modifiche.

**Chi ha chiesto il cambio.** Riscrivere i comandi ha senso solo se il pedale l'ha scelto l'utente. `PedalBox` sovrascrive `showPopup()`, `keyPressed()` e `mouseWheelMove()` per marcare l'interazione: quando il cambio arriva dal ripristino di uno stato o dal caricamento di un preset, i valori giusti sono quelli appena caricati e riscriverli li perderebbe. Prima di questa distinzione, aprire la finestra riportava i pomelli dei pedali ai valori di fabbrica, e lo stesso succedeva caricando un preset con l'interfaccia aperta.
