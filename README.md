...
**MAIN tab** — catena completa con l'**analizzatore di spettro** nell'EQ, il **display LCD** in alto e i **due caricatori IR** in basso. Sei sezioni hanno sotto al titolo la **tendina dei pedali**, con un elenco unico diviso per categoria.
...
- **Nine amp sims** — GEAR SX, MARCHELLOW, **MAUSE RECTIFIER**, plus SOLDERANO, BUGNER, DizBenZA, MAZZALARGA, BRUNTELLI and Mause Mark 5, the last six sharing one control pool and naming the head they follow next to the selector. MAUSE RECTIFIER, a two-channel American high-gain head with per-channel controls, three modes per channel, switchable tube/silicon rectifier and a Bold/Spongy variac.
- **Selectable pedals** — eleven sections are *slots* rather than fixed effects: six on the MAIN tab and five on the FX tab (DELAY, CHORUS, FLANGER, REVERB, TREMOLO), each family drawing from its own list, and every FX category opening with the effect that section always had. The same effect cannot sit in two sections. In detail: a dropdown under each section title picks the pedal that runs there, from one shared list grouped by category (Overdrive, Distortion, High Gain, Fuzz, Booster, Gate / Noise, Equalizer, Compressor). The section repopulates with exactly the knobs and switches that pedal has, and the panel resizes to fit. **Its title and enable button take the pedal's name too**, while what belongs to the slot stays put: its place in the chain and its on/off state. The spectrum analyser travels with the equaliser and the gain-reduction meter with the compressor.
- **Full FX chain**:
