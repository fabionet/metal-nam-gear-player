...
- **Nine amp sims** — GEAR SX, MARCHELLOW (JCM800 2203), **MAUSE RECTIFIER**, and six more sharing one control pool: SOLDERANO (Soldano SLO-100), BUGNER (Bogner Ecstasy 101B), DizBenZA (Diezel VH4), MAZZALARGA (Mezzabarba Trinity), BRUNTELLI (Brunetti XL R-Evo) and Mause Mark 5 (Mesa/Boogie Mark V, with the five-slider graphic EQ). Each one names the head it follows next to the selector. MAUSE RECTIFIER, a two-channel American high-gain head with per-channel Gain / Treble / Mid / Bass / Presence / Master, three modes per channel (Clean / Vintage / Modern), a tube-or-silicon rectifier and a Bold/Spongy variac. Both the power-supply sag and the per-channel presence placement are modelled, because both are audible: 1.8 dB of sag on Spongy against none on silicon, and the same presence knob giving +8.4 dB on channel 1 (feedback loop) against +1.1 dB on channel 2 (preamp).
- **Selectable pedals** — eleven sections are *slots* rather than fixed effects. Six on the MAIN tab (NGATE, GATE, COMP, OVERDRIVE, DISTORTION, EQ) draw from one list, and the five on the FX tab (DELAY, CHORUS, FLANGER, REVERB, TREMOLO) from a second list of their own, each list carrying two series — one built on the Japanese maker's schematics (craft series included) and a second gathering the other schools: the rodent's op-amp clipper, the four cascaded stages of the New York fuzz school, the two-band American studio overdrive and the very quiet British custom-shop circuits — delays, modulations and reverbs, each category opening with the effect that section always had. The same effect cannot sit in two sections: pick one that is already in use and the choice is refused with a message saying where it is. In detail: a dropdown under each section title picks the pedal that runs there, from one shared list grouped by category (Overdrive, Distortion, High Gain, Fuzz, Booster, Gate / Noise, Equalizer, Compressor). The section repopulates with exactly the knobs and switches that pedal has, with its own names, units and factory values, and the panel resizes to fit. **Its title and enable button take the pedal's name too** — put a compressor where the equaliser was and the section reads COMPRESSOR — while what belongs to the slot stays put: its place in the chain and its on/off state, so a section that was switched off stays off. The spectrum analyser travels with the equaliser and the gain-reduction meter with the compressor, appearing in whichever section you moved them to. The topologies follow published circuit schematics — soft clipping inside the feedback loop for the overdrive family, hard clipping to ground for the distortion family, peak detector split from the gain smoother for the compressors.
- **Full FX chain**:
  ...
  - Post-cab: Delay, Chorus, Flanger, Reverb, Tremolo — each one a pedal slot with its own list (analog / digital / modulated delays, CE-style chorus and ensemble, vibrato, BF-style flanger with Ultra mode, room / hall / plate / spring reverb, tremolo with wave shaping and a step slicer)
...
- **EQ spectrum analyser** — real-time FFT with the EQ response curve drawn on top and one draggable handle per band. The handles follow the EQ model chosen in the section: five for the amp tone stack (the MID band also takes frequency on the horizontal axis and Q on the mouse wheel), seven fixed frequencies for the graphic EQ — dragging one moves the matching band fader and vice versa — two sweepable ones for the parametric. The analyser lives with the equaliser rather than with one fixed section: move the equaliser and it follows, and the spectrum is tapped right after whichever section holds it. The curve is computed with the same filter coefficients as the audio path, so it cannot drift from what you hear.
